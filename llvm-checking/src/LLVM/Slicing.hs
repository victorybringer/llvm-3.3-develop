...
module LLVM.Slicing (
  ...
  -- * Print Debug Info.
  printDbgInfo, printFuncLines, getFuncLines,
  -- * Print Metrics
  printMetrics
 ) where
...
genSliceTableWith method isPar m
  ...
  | elem method ["Sym2","sym2"] = BwdSym.genSliceTable isPar m   -- Sym.genSliceTable isPar m  
  ...
  | otherwise  = error "Unknown the slice method (Symbolic,SDG,IFDS,InfoFlow or Weiser)!"
...
--  putStrLn (show bwdSliTbl)
 where
   ...
   procValDepMap  =  Sym._procValueDepSumm summ  
   instDepMap =  Sym._valueDepSummary summ  
...
----
printFuncLines :: Module -> IO ()
printFuncLines m = do
  let res = "FunctionSrcLineRange = " ++ show (getFuncLines m)
  putStrLn res
