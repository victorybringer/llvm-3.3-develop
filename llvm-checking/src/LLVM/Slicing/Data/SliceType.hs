...
--import LLVM.Slicing.Util.SrcLineNum (toSrcLnStr)
...
--showSrcLnSliTbl = showSlices2 . M.map (toSrcLnStr valMap)

showMapWith :: Show b => String -> (a -> [b]) -> Map String a -> String
showMapWith header toList ds = showMapWith2 header M.toAscList toList ds
--  if M.null ds then "\nThe final table is NULL !" 
--  else  header ++ "\n------------------------------\n" ++ res
-- where res = unlines . buildTable $ mapSnds (concat . (intersperse ","). (map show). toList) ds'
--       ds' = M.toAscList ds

showMapWith2 :: (Show a1, Foldable t) => String -> (t a2 -> [(String, a3)]) -> (a3 -> [a1]) -> t a2 -> String
showMapWith2 header mapToList valToList ds = 
  if null ds then "\nThe final table is NULL !" 
  else  header ++ "\n------------------------------\n" ++ res
 where res = unlines . buildTable $ mapSnds (concat . (intersperse ","). (map show). valToList) ds'
       ds' = mapToList ds

avgSize :: SliceTable -> Int
avgSize m = round $ (allSlices m)/(fromIntegral $ M.size m)  
allSlices = fromIntegral . sum . map IS.size . M.elems

showSliSize :: SliceTable -> String
