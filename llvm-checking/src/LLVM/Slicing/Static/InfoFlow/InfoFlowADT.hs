...
getInfoRel sv ds vs = (iDs,ivi,iiv,ivv)  
  where  ivi = if null svRefs then R.emptyRel 
               else R.mkRel [(v,svID)| v <- svRefs ]
         iiv = if null ds then R.emptyRel
               else R.mkRel [(n,v) | n <- svRefIDs, v <- ds] 
                 --  R.mkRelNeighbors svID ds        
         ivv0 = if null svRefs || null ds then R.emptyRel 
                else R.mkRel [(v1,v2) | v1 <- svRefs, v2 <- ds] 
         ivv = S.union ivv0 . R.identityRel $ vs S.\\ iDs
         iDs = S.fromList ds
...
updateInfoRel si@(SInfo viM ivM vvM dM pM) i (iDs,ivi,iiv,ivv) = do   
   ...
   let  iID = instructionUniqueId i 
        ...
        ivi' = S.union ivi0 $ R.comp ivi ivv0    -- ivi 
        iiv' = S.union iiv $ R.comp ivv iiv0
        ivv' = R.comp ivv ivv0       
  ...
  case valueContent' fv of
   FunctionC f -> do
     let fID = functionUniqueId f 
         fSumms = IM.findWithDefault R.emptyRel fID summMap
...
--         vSumm v = [ b | (a,b) <- fSumms', a == v ] 
     ...
     let 
         ...
         funcVVs = S.union actFmlVV $ R.comp fVV0 actFmlVV    -- allVV0
                -- R.transClose $ S.union actFmlVV allVV0  
         actFmlVV = R.mkRel fActFml  
         fmlActVV = S.union fmlActVV' $ R.comp fmlActVV' fAliaFml
         fmlActVV' = R.mkRel $ M.toList argMap'
         (actOutVar,actInVar) = ((nub $ map fst fSumms'), nub $ map snd fSumms')
         --
         findII n = flip R.comp iiv0 $ S.filter ((== n). snd) ivi0
         findIIs ns = flip R.comp iiv0 $ S.filter (flip S.member ns. snd) ivi0
         ...
         getCallDep n = S.toList . S.unions $ iCD : iDDs 
            where iCD  = S.fromList $ map valueUniqueId iCD'
                  ...
                  iCD' = findCDInsts n
     let 
         fVIs2 = R.comp vis ivvs 
            where vis = S.union fVI0 allVI0  
                  ivvs = S.union funcVVs $ R.comp funcVVs ivv0
         fIVs2 = S.unions [iiv1,iiv2,iiv3]   -- `debug` (show $ getCallDep iID) 
            where iiv1 = R.mkRelNeighbors iID locVars     
                  iiv1' = R.mkRelNeighbors iID actInVar
                  iiv2 = R.comp ivvs $ S.union iiv1' iiv0  
                  ivvs = R.comp (S.union fVV0 allVV0) funcVVs
                  iiv3 = R.mkRel [(i,v)| i <- getCallDep iID, v <- locVars ++ fFmls ]
                  !locVars = concatMap funcLocs $ f:fCallFs  
         fVVs2 =  funcVVs
            where  ivvs = S.union funcVVs $ R.comp funcVVs ivv0
     ...
     let 
         ...
         ivi = S.unions [ivi0,ivi1,ivi2,ivi2']  -- [ivi0,ivi2']
            where ivi1 = R.mkRel [(v,iID)| v <- actInVar ]   
                  ...
                  ivi2 = R.comp ivi1 ivv0
                  ivi2' = R.comp ivi1' vvs 
                  vvs = R.comp fmlActVV fVV0   -- S.union fmlActVV $ 
         iiv = S.unions [iiv1,iiv2,iiv3]   -- [iiv1,fmlIVs]   
            where iiv1 = R.mkRelNeighbors iID actOutVar 
                  iiv2 = R.comp ivv' iiv0    
                  fmlIVs = R.comp fmlActVV fIV0   
                  iiv3 = R.comp ivv' fmlIVs
         ivv = S.unions [ivv1,ivv2]     -- `debug` (show fmlActVV)
            where ivv1 = R.comp ivv' $ R.comp fmlActVV fVV0
                  ivv2 = R.comp ivv' ivv0    
         ivv' = S.unions [ivv1,ivv2]    
            where ivv1 = R.identityRel $ fVars S.\\ iDs  
...
doUnBrInfo si i bi = do
  fVars <- analysisEnvironment fAllVars
  let ivi = R.comp ivi0 ivv'
      iiv = S.union iiv' $ R.comp (R.identityRel fVars) (R.comp ivv' iiv0)
      ivv = R.comp (R.identityRel fVars) ivv' 
      iDs = lkpDs si bi   
      iiv' = R.mkRel [(iID,v)| v <- S.elems iDs]
      ivv' = R.transClose ivv0
...
computeInfoRel' si@(SInfo viM ivM vvM dM pM) i = do
  ...
  let iviB = if null iRefs then R.emptyRel 
             else R.mkRel [(v,iID)| v <- iRefs ]
      iivB = if null iDefs then R.emptyRel
             else R.mkRelNeighbors iID iDefs 
      ivvB = S.union ivv0 $ R.identityRel iPsB 
      ivv0 = if null iRefs || null iDefs then R.emptyRel 
             else R.mkRel [(v1,v2) | v1 <- iRefs, v2 <- iDefs]
      iDsB = S.fromList iDefs
      iPsB = fVars S.\\ iDsB
  let iviAB = S.union iviA $ R.comp iviB ivvA    -- ivi 
      iivAB = S.union iivB $ R.comp ivvB iivA
      ivvAB = R.comp ivvB ivvA 
...
getUnBrIR si bi vs = (iDs,ivi,iiv,ivv)
  where  ivi = S.union ivi0 $ R.comp ivi0 ivv'
         iiv = S.union iiv0 $ R.comp (R.identityRel vs) (R.comp ivv' iiv0)
         ivv = S.union ivv0 $ R.comp (R.identityRel vs) ivv' 
         iDs = lkpDs si bi
         ivv' = R.transClose ivv0
         (ivi0,iiv0,ivv0) = (lkpVI si bi,lkpIV si bi, lkpVV si bi)
{-# INLINE getUnBrIR #-}
...
getBrIR si@(SInfo viM ivM vvM dsM _) sv bIs vs = (iDs,ivi,iiv,ivv)
  where  (ivi,iiv,ivv) = (S.union ivi' ivi0, S.union iiv' iiv0,
                          S.union ivv' ivv0)
         ivi' = if null svRefs then R.emptyRel 
                else R.mkRel [(v,svID)| v <- svRefs, svID <- svIDs ]
         iiv' = if null ds then R.emptyRel
                else R.mkRel [(svID,v)| v <- ds, svID <- svIDs ] 
         ivv' = S.union ivv2 . R.identityRel $ vs S.\\ iDs
         ivv2 = if null svRefs || null ds then R.emptyRel 
                else R.mkRel [(v1,v2) | v1 <- svRefs, v2 <- ds] 
...
computeSummEdge si f = R.mkRel $ concatMap getSummEdge fFmls
  where fID = functionUniqueId f 
        fFmls = mapMaybe toVarName $ functionParameters f    -- map toVarName'
        ...
        getSummEdge ai = [(ai,bi) | isArgOut, bi <- argInNodes]
           where ais = ai : (toAlias ai)
                 reachNodes = S.elems. R.dom $ R.sliceB ais fVVs                 
                 argInNodes = intersect fFmls reachNodes 
                 isArgOut = not . null $ reachNodes \\ ais 
