...
instance Semigroup SliceSummary where
  (SliceSummary ir1 se1 tr1) <> (SliceSummary ir2 se2 tr2) =
    SliceSummary (IM.unionWith mrgInfoRel ir1 ir2)(IM.unionWith S.union se1 se2)(tr1 + tr2)
        ...
        where toAlias ai = maybeToList $ M.lookup ai fAliasMap
              getSummEdge ai = [(ai,bi) | isArgOut, bi <- argInNodes]   
               where !ais = toAlias ai    
                     ais' = ai:ais
                     reachNodes = S.elems. R.dom $ R.sliceB ais' fVVs   -- fVVs'                 
                     argInNodes = intersect fFmls reachNodes                            
                     bwdSli vs = R.domWith (S.fromList vs) fIVs 
                     isArgOut = not argIn  
                       where argIn = if null ais then argIn1 && argIn2
                                     else argIn2 && argIn3  
                             argIn1 = null $ reachNodes \\ [ai]   
                             argIn2 = S.null $ bwdSli [ai]
                             argIn3 = (S.size $ bwdSli ais) == length ais 
