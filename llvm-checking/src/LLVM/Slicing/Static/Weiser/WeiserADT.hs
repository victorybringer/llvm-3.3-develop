...
lkpRCWith :: IntMap ValueIds -> SliceInfo -> Instruction -> ValueIds
lkpRCWith rcM si i = {-# SCC lkpRCWith #-}  IS.union (lkpRC si i) iRC   
  where  iID = instructionUniqueId i
         iRC = IM.findWithDefault IS.empty iID rcM  
{-# INLINE lkpRCWith #-}
