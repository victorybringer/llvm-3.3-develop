...
instance Semigroup SliceSummary where
  (SliceSummary bs1 ms1 tr1) <> (SliceSummary bs2 ms2 tr2) =
...
--                   , allRcMap = rcm
      ...
      analysis = bwdDataflowAnalysis top meetSliceInfo sliceTransfer  -- bwdDataflowAnalysis
  ...
  let trStr = fName ++ fStrLn ++ "\'s #Insts_traced = " ++ show tr -- (length tr)
      fStrLn = case getFunctionLine f of
               ...
               Nothing        -> ""
      si@(SInfo sc' rcm') = dataflowResult localInfo
  return $ (bwdSlice .~ sc')
         $ (traceSize %~ (+) tr)
         $! (rcSummary %~ IM.unionWith IS.union rcm') s   -- `debug` trStr
  where
...
computeSlice m vs = res  -- `showGraph` (cg,mName)
  where
    cg = callGraph m pta []
    funs = callGraphFunctions cg   -- FGL.topsort' $ definedCallGraph cg  
    pta = runPointsToAnalysis m
    funModSets = computeModSet funs    
--    analyses :: [ComposableAnalysis SliceAnalysis Function]
--    analyses = [ identifySlice vs m sliceSumm ]
