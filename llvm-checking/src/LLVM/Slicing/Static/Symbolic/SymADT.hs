{-# LANGUAGE BangPatterns,ViewPatterns,DeriveGeneric,TemplateHaskell,NoMonomorphismRestriction,
             RankNTypes,FlexibleContexts,FlexibleInstances,DeriveAnyClass, TypeFamilies,
             DeriveGeneric,StandaloneDeriving,TypeSynonymInstances #-}
{-# OPTIONS_GHC -XUndecidableInstances -funbox-strict-fields -rtsopts #-}
...
data SliceEnv' a b = SEnv {   --  procSliTbl :: !(IntMap SliceTable),  
                       procValueDep :: !(IntMap (VDTable a b))
...
data SliceSummary' a b = SliceSummary {  
                      _procValueDepSumm :: !(IntMap (VDTable a b))
                    , _valueDepSummary :: !(VDTable a b)
                    , _traceSize :: Int 
                    } deriving (Show,Generic,NFData)

$(makeLenses ''SliceSummary')

instance (Ord b,IsValueDepTable a b) => Eq (SliceSummary' a b) where
...
----------------------------------
------
unionLs :: (IsValue v, IsValueDepTable a b, VSElm b ~ Int, VDKey a ~ Int) => 
...
----
addSliInfo :: (IsValue v, IsValueDepTable a b,VDKey a ~ Int) => 
...
setTrSliInfo :: (IsValueDepTable a b,VSElm b ~ Int,VDKey a ~ Int) => IntMap ValueIds  
             -> Instruction -> SliceInfo' a b -> (Analysis' a b) (SliceInfo' a b)  
setTrSliInfo !cdM i !si = addTrSliInfo i l' si
   where   !l' = unionLs cdM si i i

addTrSliInfo :: (IsValue v,IsValueIdSet b,IsValueDepTable a b,VDKey a ~ Int)
             => v -> b -> SliceInfo' a b -> (Analysis' a b) (SliceInfo' a b)
...
---
setSliInfo :: IsValueDepTable a b => VDTable a b -> SliceInfo' a b -> (Analysis' a b) (SliceInfo' a b)
setSliInfo !vdM si = return $! SInfo vdM

setWithSliInfo :: IsValueDepTable a b => VDTable a b -> SliceInfo' a b -> (Analysis' a b) (SliceInfo' a b)
setWithSliInfo !vdM si = return $! modifySInfo (mrgValueDep vdM) si


--------------------------
----
modifySInfo :: IsValueDepTable a b => (VDTable a b -> VDTable a b) -> SliceInfo' a b -> SliceInfo' a b
modifySInfo modFn (SInfo vdM) = SInfo (modFn vdM)    
--
updSInfo,xtdSInfo :: IsValueDepTable a b => VDKey a -> b -> SliceInfo' a b -> SliceInfo' a b
updSInfo n !ds = modifySInfo (updValueDep n ds)  
xtdSInfo n !ds = modifySInfo (xtdValueDep n ds) 

updsSInfo,xtdsSInfo :: IsValueDepTable a b => [(VDKey a,b)] -> SliceInfo' a b -> SliceInfo' a b
updsSInfo !nds = modifySInfo (updsValueDep nds)
xtdsSInfo !nds = modifySInfo (xtdsValueDep nds)
    
updsSInfo2,xtdsSInfo2 :: (IsValueDepTable a b,VDKey a ~ Int) => 
                ValueIds -> b -> SliceInfo' a b -> SliceInfo' a b
