{-# LANGUAGE BangPatterns,ViewPatterns,DeriveGeneric,TypeSynonymInstances,
             RankNTypes,MultiParamTypeClasses,TemplateHaskell,CPP #-}
 

module LLVM.Slicing.Static.SDG.IFDS.IFDSSlicer (
 -- * Types
 SliceSummary(..),
 -- * Slice Computing
 computeSlice,
...
import LLVM.Slicing.Static.SDG.IFDS.SummaryEdge
...
instance Semigroup SliceSummary where
  (SliceSummary ns1 es1 fms1 se1 tr1) <> (SliceSummary ns2 es2 fms2 se2 tr2) =
    SliceSummary (IM.union ns1 ns2)(M.union es1 es2)(IM.unionWith HS.union fms1 fms2)
                 (IM.unionWith S.union se1 se2)(tr1 + tr2)
...
identifySlice m lns =   -- flip showGraph (icfg,mName,"ICFG") $
  composableAnalysisM runner (sliceAnalysis m summEdgeMap) lns
  where
    runner a = runAnalysis a constData cache
    constData = SEnv undefined mempty mempty mempty 
    cache = SState 0 mempty
    summEdgeMap = genSummEdgeMap (SDGSummEdge valMap) icfg   
...
sliceAnalysis :: (FuncLike funcLike, HasCFG funcLike,HasFunction funcLike)
               => Module -> SummEdgeMap -> funcLike -> SliceSummary -> Analysis SliceSummary
sliceAnalysis m sem funcLike s@(SliceSummary _ _ fms se _ ) = do 
  ...
  let trStr = fName ++ fStrLn ++ "\'s #Insts_traced = " ++ show tr -- (length tr)
      ...
      fSummEdge =  -- timeIt2 (fName ++ "_IFDS-SummEdge"). 
           S.fromList. mapMaybe toIFDSSumm. S.toList $ IM.findWithDefault S.empty fID sem 
         where  toIFDSSumm (argIn,argOut) = do 
                   inN <- IM.lookup argIn fParaMap
                   outN <- IM.lookup argOut fParaMap
                   let outDel = isDelOut (fst outN)
                   case (toFstValId inN,toFstValId outN) of 
                     ( (_,-2), _ ) -> Nothing
                     ( _, (_,-2) ) -> Nothing
                     ( (i,-1),(o,-1) ) -> return ((- getNewId fID o, getNewId fID i),outDel)
                     ( (i,-1),(o,_) ) -> return ((- o, getNewId fID i),outDel)
                     ( (i,_),(o,-1) ) -> return ((- getNewId fID o, i),outDel)
                     ( (i,_),(o,_) ) -> return ((- o, i),outDel)
                toFstValId (v,n) = (valueUniqueId v, n)   
                isDelOut vi = not . HS.member vi $ IM.findWithDefault HS.empty fID fms'
  return $ (funModSet .~ fms') 
         $ (funSummEdges %~ IM.insert fID fSummEdge)
         $ (nodeSummary %~ IM.union nsM) 
         $ (traceSize %~ (+) tr)
         $! (edgeSummary %~ M.union (M.union esM outEdges)) s  --  `debug` trStr
  where
...
genSDG isPar m = sdg  `debug` (trStr ++ sdgSize)
  where -- sdg = G.delNodes delNodes $ G.mkGraph labNodes (labEdges ++ aliasEdges)
         ...
         trStr = "\n\tIts trace Info: #Insts_IFDS = " ++ show tr    
