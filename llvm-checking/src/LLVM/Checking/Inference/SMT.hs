...
module LLVM.Checking.Inference.SMT 
-- (computeInducedFacts,
--   computeLoopFormula,  -- zyz
--   isSat,
--   ignoreCasts) 
where
...
-- zyz
computeLoopFormula :: (HasFunction funcLike, HasBlockReturns funcLike,
                        HasCFG funcLike, HasCDG funcLike, HasDomTree funcLike)
                    => funcLike
                    -> BasicBlock
                    -> Instruction
                    -> FormulaBuilder (Maybe (SInt32 -> SBool))
computeLoopFormula funcLike bb0 target
      ...
      case cdep of
        BranchInst { branchTrueTarget = tt
          ...
          InstructionC ICmpInst { cmpPredicate = p
            ...
            | isPhiNode (ignoreCasts val1)  ||
              isPhiNode (ignoreCasts val2) -> do
...
inducedFact val1 val2 p doNeg = do
  ...
  case (valueContent' val1, valueContent' val2) of
    ...
    -- had from before.
    _ -> return $ const sTrue  -- zyz
      --  fail "Cannot produce a fact here"    -- zyz
...
cmpPredicateToRelation p =
  case p of
    ...
    ICmpSle -> return (.<=)
    _ ->  Nothing    -- zyz
      --  fail "cmpPredicateToRelation is a floating point comparison"
