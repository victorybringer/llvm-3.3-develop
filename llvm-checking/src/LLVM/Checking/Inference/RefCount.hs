...
{-# LANGUAGE ViewPatterns, DeriveGeneric, CPP #-}
...
module LLVM.Checking.Inference.RefCount (
  RefCountSummary(..),
  identifyRefCounting,
  -- * Testing
...
import LLVM.Checking.Inference.Finalize
import LLVM.Checking.Inference.ScalarEffects
...
instance Semigroup RefCountSummary where
  (RefCountSummary s1 a1 r1 rcts1 d1) <> (RefCountSummary s2 a2 r2 rcts2 d2) =
    RefCountSummary { _conditionalFinalizers = s1 <> s2
                    , _unrefArguments = a1 <> a2
                    , _refArguments = r1 <> r2
                    , _refCountedTypes = HM.unionWith HS.union rcts1 rcts2
                    , _refCountDiagnostics = d1 <> d2
                    }
