...
module LLVM.Checking.Inference.Output (
  -- * Interface
  OutputSummary(..),
  identifyOutput,
...
instance Semigroup OutputSummary where
  (OutputSummary s1 sf1 d1) <> (OutputSummary s2 sf2 d2) =
    OutputSummary (M.union s1 s2) (M.union sf1 sf2) (d1 <> d2)
...
outAnalysis m funcLike s = do
  let envMod e = e { moduleSummary = s
      ...
      analysis = fwdDataflowAnalysis top meetOutInfo (outTransfer m)
  ...
  case M.lookup arg (info ^. lns) of
    ...
    Just (ArgIn, ws) ->
      case newVal of
        ...
        --   in (lns %~ M.insert arg (ArgBoth, S.insert nw ws)) info
        ArgIn -> info
        ArgBoth -> error "LLVM.Checking.Output.merge(2): Infeasible path"
...
outputSummaryToTestFormat (OutputSummary s sf _) =
  ...
  where
    ...
    flattenArg [] = error "LLVM.Checking.Output.outputSummaryToTestFormat: groupBy made an empty group"
