...
--- {-# OPTIONS_GHC -O -rtsopts -funbox-strict-fields #-}
-- | An interprocedural may-be-null pointer analysis.
module LLVM.Checking.Inference.INullPtrs (
  INullPtr,
  valueInjectsNull, valueMatchesNull,
  nullPtrTest,
  getNullPtrs, funcNullPtrs  
  ) where
...
import LLVM.Slicing.Static.SDG.IFDS.IFDS
...
inullFlow _ Nothing i@AllocaInst {} _
  ...
  | isPointerPointerType (toValue i) = [Nothing, Just $ toValue i]
  | otherwise = [Nothing]
...
inullPassArgs _ Nothing (CallInst { callArguments = cargs }) f =
  ...
  in Nothing : map (Just . toValue . snd) nullArgs
...
inullPassArgs _ v@(Just v') (CallInst { callArguments = cargs}) f =
  ...
  in case isGlobal v' of
    True -> v : map (Just . toValue) nullFrmls
    False -> map (Just . toValue) nullFrmls
  where
...
inullExternReturnVal _ Nothing _ ci =
  case isPointerType (toValue ci) of
    False -> [Nothing]
    True -> [Just (toValue ci), Nothing]
...
inullEntrySetup _ m entry = Nothing : map Just (gvs ++ evs)
  where
    ...
    gvs = filter isPtrToPtr $ map toValue globalsWithoutInits
    evs = filter isPtrToPtr $ map toValue (moduleExternalValues m)
...
nullPtrTest :: Module -> Set String
nullPtrTest m = S.filter isNotNumericName $ S.map (show . fromJust . valueName) endVals
  where
    pta = runPointsToAnalysis m   
    initps = case findMain m of
       Just mf -> [mf]
       Nothing -> take 1 $ moduleDefinedFunctions m 
    progMain = head initps 
    icfg = mkICFG m pta [progMain]
    analysis = INullPtr
    res :: IFDSResult Value
    res = ifds analysis icfg
    retInst = fromJust $ functionExitInstruction progMain
    endVals = maybe S.empty id (ifdsInstructionResult res retInst)
    isNotNumericName = not . (=~"%[[:digit:]]+")

getNullPtrs :: Module -> Set Value
getNullPtrs m = endVals
  where
    pta = runPointsToAnalysis m 
    initps = case findMain m of
       Just mf -> [mf]
    ...
    res :: IFDSResult Value
    res = ifds analysis icfg
    retInst = fromJust $ functionExitInstruction (head initps)
    endVals = maybe S.empty id (ifdsInstructionResult res retInst)

funcNullPtrs :: Module -> Function -> Set Value
funcNullPtrs m f = endVals
  where
    pta = runPointsToAnalysis m 
    initps = case findMain m of
       Just mf -> [mf]
