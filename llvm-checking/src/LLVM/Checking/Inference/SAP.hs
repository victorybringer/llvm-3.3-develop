...
-- in a Module.
module LLVM.Checking.Inference.SAP (
  SAPSummary(..),
  identifySAPs,
  finalizedPaths,
  returnedPaths,
...
import LLVM.Slicing.Util.Utils (getFuncName,flattenValue)

import LLVM.Checking.Inference.Finalize
import LLVM.Checking.Inference.SAPPTRel hiding (_sapDiagnostics)
...
instance Semigroup SAPSummary where
  (SAPSummary r1 a1 f1 c1 d1) <> (SAPSummary r2 a2 f2 c2 d2) =
    SAPSummary { _sapReturns = M.union r1 r2
               , _sapArguments = M.unionWith HS.union a1 a2
               , _sapFinalize = M.unionWith HS.union f1 f2
               , _sapReturnContainer = M.unionWith HS.union c1 c2
               , _sapDiagnostics = d1 <> d2
               }
