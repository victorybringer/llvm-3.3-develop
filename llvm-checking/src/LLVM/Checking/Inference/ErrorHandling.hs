{-# LANGUAGE ViewPatterns, RankNTypes, ScopedTypeVariables,FlexibleContexts #-}
...
--    pattern.
module LLVM.Checking.Inference.ErrorHandling (
  ErrorSummary(..),
  errorDescriptors,
  ErrorDescriptor(..),
  ErrorReturn(..),
...
instance Semigroup ErrorSummary where
  (ErrorSummary m1 b1 d1 s1 f1) <> (ErrorSummary m2 b2 d2 s2 f2) =
    ErrorSummary (HM.union m1 m2) (b1 <> b2) (d1 <> d2) (s1 <> s2) (f1 <> f2)
...
instance Semigroup ErrorState where
  (ErrorState c1 s1 fc1) <> (ErrorState c2 s2 fc2) =
    ErrorState { errorCodes = c1 <> c2
               , successModel = HM.unionWith M.union s1 s2
               , formulaCache = HM.union fc1 fc2
               }
