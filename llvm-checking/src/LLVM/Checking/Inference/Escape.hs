...
module LLVM.Checking.Inference.Escape (
  EscapeSummary(..),
  identifyEscapes,
  instructionEscapes,
...
import LLVM.Slicing.Util.Utils (flattenValue,getFuncName)
...
-- zyz
instance Generic (SolvedSystem a b) where
...
toSink e = error ("LLVM.Checking.Escape.toSink: Unexpected non-constructed term: " ++ show e)
