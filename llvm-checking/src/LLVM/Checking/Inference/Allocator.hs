...
-}
module LLVM.Checking.Inference.Allocator (
  AllocatorSummary(..),
  identifyAllocators,
  -- * Testing
...
instance Semigroup SummaryType where
  (ST f1 a1) <> (ST f2 a2) = ST (f1 <> f2) (a1 <> a2)
...
instance Semigroup AllocatorSummary where
  (AllocatorSummary s1 d1 f1) <> (AllocatorSummary s2 d2 f2) =
    AllocatorSummary (s1 <> s2) (d1 <> d2) (f1 <> f2)
