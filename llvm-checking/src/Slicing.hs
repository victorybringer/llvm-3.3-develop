...
--     Just _   -> error "Unknown the graph type (Sdg,Cg,Cdg,Cfg,Icfg,Pdt,Dt, or All)!"
     
    -- compare slice tables 
    let cmpOpt = cmpSlices opts
        cmps = case cmpOpt of 
          []  -> ""  -- compareSliceMethod m ("Symbolic","IFDS")    -- SDG
          [md1]  -> compareSliceMethod m (md1, "Symbolic")
          (md1 : md2 : _) -> compareSliceMethod m (md1,md2)
    if not (null cmpOpt) 
       then writeFile (inFile ++ ".SliceDiffResult") cmps 
       else return () 
