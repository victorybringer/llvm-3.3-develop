  ...
  where
    go acc pfxs uname =
      case (pfxs, uname) of
        ...
        ([], CtorDtorName _) -> return $! singleton ' '    -- zyz
                -- error "Illegal fallthrough in constructor/destructor case"
...
showPrefix pfx =
  case pfx of
    ...
    -- zyz
    TemplateArgsPrefix s -> showTArgs s
    TemplateParamPrefix s -> showTParam s 

-- zyz
showTParam ::  TemplateParam -> Pretty Builder
showTParam (TemplateParam (Just n)) =  return $! fromString (show n)  
showTParam (TemplateParam Nothing) = return $! singleton ' '
...
showUnqualifiedName uname =
  case uname of
    ...
    CtorDtorName _ ->  return $! singleton ' '    -- zyz
        --  error "showUnqualifiedName shouldn't reach the ctor/dtor case"
