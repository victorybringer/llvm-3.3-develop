{-# LANGUAGE TypeSynonymInstances, FlexibleInstances, MultiParamTypeClasses, RankNTypes,FlexibleContexts #-}
{-# LANGUAGE ViewPatterns, DeriveGeneric, TemplateHaskell, CPP #-}
...
instance Semigroup FinalizerSummary where
  (FinalizerSummary s1 d1) <> (FinalizerSummary s2 d2) =
    FinalizerSummary (HM.unionWith merge s1 s2) (d1 <> d2)
    where
